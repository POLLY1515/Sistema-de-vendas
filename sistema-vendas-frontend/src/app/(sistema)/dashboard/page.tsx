    ...
    return [
      {
        ...
        descricao: "Valor consolidado das vendas consideradas no resumo.",
      },
      {
        titulo: "Total de pedidos",
        valor: String(resumo.totalPedidos),
        descricao: "Quantidade de pedidos registrados no sistema.",
      },
      {
        titulo: "Ticket medio",
        valor: formatarMoeda(resumo.ticketMedio),
        descricao: "Valor medio das vendas consideradas.",
      },
      {
        titulo: "Pedidos em aberto",
        valor: String(resumo.pedidosEmAberto),
        descricao: "Pedidos que ainda precisam de acompanhamento.",
      },
    ];
  }, [resumo]);
  ...
  return (
    <main className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          ...
          >
            ...
            <p className="mt-2 text-xs text-slate-500">{card.descricao}</p>
          </article>
        ))}
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.4fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <span className="text-sm text-slate-500">
              {resumo?.vendasHoje ?? 0} venda(s) hoje
            </span>
          </div>

          <div className="space-y-3">
            {resumo?.resumoPorStatus.length ? (
              resumo.resumoPorStatus.map((item) => (
                <div
                  key={item.status}
                ...
                >
                  ...
                  <p className="text-sm text-slate-500">
                    {item.quantidade} pedido(s) neste status.
                  </p>
                </div>
              ))
            ) : (
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-lg font-bold text-slate-900">Ultimas vendas</h2>

          {resumo?.ultimasVendas.length ? (
